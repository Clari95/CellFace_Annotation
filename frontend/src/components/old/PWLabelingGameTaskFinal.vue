    ...
    <v-row fill-width>
      ...
      <v-col class='mx-auto mt-0 main'>
          <v-row class='canvas-wrapper'>
            ...
            <div class='canvas'>
              <div v-if="allProperties.currentLevel == 4" >
              <v-subheader class="pl-3 level"
              style="font-size:1.2em">
                <b class="teal--text">Level 4: <br/>
                  <p style="font-size:0.8em">You finished Training! Accuracy is counting</p>
                  </b></v-subheader>
              </div>
              <div v-if="allProperties.currentLevel == 5">
              <v-subheader class="pl-3 level"
              style="font-size:1.2em">
                <b class="teal--text">Level 5: <br/>
                  <p style="font-size:0.8em">You finished Training! Accuracy is counting</p>
                  </b></v-subheader>
              </div>
            ...
            <div class='rightside'>
              <v-card-text class='text-center' v-if="allProperties.labelingApproach === 'basic'">
              ...
              <v-card-text class='text-center' v-if="allProperties.labelingApproach === 'game'">
                user: <b>{{allProperties.UserID}}</b> <br/>
                GAME!!! <br/>
                ...
                <v-dialog v-model='dialog' width='1000'>
                  ...
                  <v-card>
                    ...
                    <v-card>
                      ...
                      <ul>
                        <li>never annotate one area with two labels (not implemented right now)</li>
                        <li>
                        ...
                        <li>
                          ...
                          <ul>
                            <li>OrgImage: On/Off; changeing intenstiy</li>
              ...
              <v-col cols="12" v-if="allProperties.labelingApproach === 'game'">
                <v-card dense elevation='10' width="180" class="scores">
                  <v-card-text class='text-center'>
                      ...
                      added areas: <b>{{addedAreas}}</b>
                      <br />Time:
                      <b>{{this.seconds}} seconds</b>
                      Points:
                      <b class="display-3 font-weight-bold teal--text">
                        {{numberCells * 10 + numberRegions * 30 +
                        addedAreas * 50 - minusPoints}}</b><br/>
                      Highscore:
                      <b class="font-weight-bold red--text">
                        {{highscore}}
                      </b>
                  </v-card-text>
                </v-card>
      ...
      <div class='nav-arrow-next'>
            ...
            updateScores(allProperties.labelingApproach);"
            color='primary'>
...
export default {
  ...
  methods: {
    ...
    uploadData(section) {
      ...
      console.log(`sen level ${this.allProperties.currentLevel}`);
      this.labelAreas = String(this.labelAreas);
      this.labelAreasRegion = String(this.labelAreasRegion);
      HTTP.post(
        // 'http://localhost:3000/labelmask/'
        `label/${this.labelAreas}/${this.labelAreasRegion}/${this.imageRef_num}/${this.allProperties.UserID}/${this.allProperties.dataset_typ}/${this.allProperties.currentLevel}`,
      ).then(async (response) => {
        ...
        // if (section === 'next_image') {
        console.log(`wrong albel: ${response.data.wrong_label}`);
        console.log(this.wrong_label);
    ...
    uploadNewData() {
      ...
      HTTP.post(
        // 'http://localhost:3000/labelmask/'
        `newAreas/${this.newAreas}/${this.imageRef_num}/${this.allProperties.UserID}/${this.allProperties.dataset_typ}/final`,
      ).then(async (response) => {
        // this.newImage = response.data.image;
    ...
    getData(mounted) {
      ...
      this.newImage = null;
      this.newImageMask = null;
      console.log(this.imageRef_num);
      console.log(this.image_section);
      console.log(this.allProperties.labelingApproach);
      HTTP.post(
        `getData/${this.imageRef_num}/${mounted}/${this.allProperties.UserID}/${this.image_section}/${this.allProperties.dataset_typ}/${this.allProperties.labelingApproach}/${this.allProperties.currentLevel}`,
      ).then(async (response) => {
    ...
    updateScores(tooltyp) {
      ...
      const AccuracyAll = ['test', 'test', 'test', 'test'];
      const Scores = [this.numberCells, this.numberRegions, wrongAreas,
      ...
      HTTP.post(
        `updateScoreGame/${tooltyp}/${this.allProperties.UserID}/${this.allProperties.dataset_typ}/${allScores}/${date}`,
    ...
    /* getDataTutorial() {
      ...
      HTTP.post(
        ...
        this.disabled_undo = false;
        this.text_button = '';
        this.celltyp = this.allProperties.dataset_typ;
        // this.getCelltyp(celltyp);
      }).catch((e) => {
        console.warn(e);
      });
  ...
  mounted() {
    console.log(`level child ${this.allProperties.currentLevel}`);
    /* if (this.allProperties.currentLevel === 5) {
      this.imageRef_num = 11;
