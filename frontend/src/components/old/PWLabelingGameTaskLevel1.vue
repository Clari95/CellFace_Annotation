      ...
      <v-col class='mx-auto mt-0 main'>
          <v-row class='canvas-wrapper' v-if="bounce_nextLevel === false">
            ...
            <div class='canvas'>
              ...
              style="font-size:1.2em">
                <b class="teal--text">Level 1: <br/>
                  <p style="font-size:0.8em">Label all marked areas
                    (excluding cells that are cut off from the image) <br/>
                    with a left click!
                    Explore different image views!
                  </p>
...
// const myTrack = new Audio('require(`../assets/audio/poker-chips.mp3`)');
// require('howler');
...
export default {
  ...
  data: () => ({
    ...
    // three images in this level could be 5
    numberImagesLevel: 3,
    nextLevelText: '',
    nextLevelprops: {
      bounce: true,
  ...
  methods: {
    ...
    // upload labels
    uploadData(section) {
      console.log('uploadData start Level 1');
      ...
      HTTP.post(
        ...
        `label_level/${this.labelAreas}/${this.labelAreasRegion}/${this.imageRef_num}/${this.allProperties.UserID}/${this.allProperties.dataset_typ}/1/${this.allProperties.labelingApproach}`,
      ).then(async (response) => {
    ...
    updateScores(all, review) {
      ...
      HTTP.post(
        ...
        // console.log('updatscores level 1');
    ...
    /* updateBreak() {
      console.log('UPDATE SCORES review');
      // const wrongAreas = this.minusPoints / 5;
      const AccuracyAll = [0, 0, 0, 0];
      // const Scores = [this.imageRef_num, this.numberCells, this.numberRegions, wrongAreas,
      // this.addedAreas, this.seconds,
      // this.allProperties.points, AccuracyAll, this.allProperties.currentLevel];
      // const Scores = [this.numberCells, this.numberRegions, this.addedAreas, this.seconds];
      // this.$emit('updateScores', Scores);
      this.$emit('updateScores', [0, 0, 0, 0, 0, 0, 0, AccuracyAll, 2, 'skip']);
      this.$emit('updateScores', [0, 0, 0, 0, 0, 0, 0, AccuracyAll, 3, 'skip']);
      this.$emit('updateScores', [0, 0, 0, 0, 0, 0, 0, AccuracyAll, 4, 'skip']);
      this.$emit('updateScores', [0, 0, 0, 0, 0, 0, 0, AccuracyAll, 5, 'skip']);
      console.log('upload scores');
      // let points = this.numberCells * 10 + this.numberRegions * 30
      // + this.addedAreas * 50 - this.minusPoints;
      const allScores = String(this.allProperties.AllScores);
      console.log('allScores for sending');
      console.log(this.allProperties.AllScores);
      const date = this.getDate();
      HTTP.post(
        `updateScoreGame/${this.allProperties.labelingApproach}/${
    ...
    /* calAccuracyAverage() {
      ...
      console.log(this.accuracyTotal.length);
      ...
      for (let i = 0; i < this.accuracyTotal.length; i += 1) {
        console.log(`i-te ${this.accuracyTotal[i]}`);
        console.log(`i-te ${this.accuracyTotal[i][0]}`);
        this.accuracyAreas += this.accuracyTotal[i][0];
        this.accuracyPixel += this.accuracyTotal[i][1];
        this.accuracyLabel += this.accuracyTotal[i][2];
      }
      console.log(this.accuracyAreas);
      this.accuracyAreas /= this.accuracyTotal.length;
      this.accuracyPixel /= this.accuracyTotal.length;
      this.accuracyLabel /= this.accuracyTotal.length;
      const accuracySpecified = [this.accuracyAreas, this.accuracyPixel, this.accuracyLabel];
      // const accuracyTotalValue = (accuracySpecified[0] + accuracySpecified[1]
      // + accuracySpecified[2]) / 3;
      this.average = (this.accuracyAreas + this.accuracyLabel) / 2;
      this.nextLevelprops.accuracyForScoring = this.average;
      console.log(`3 accuracys:  ${accuracySpecified}`);
      console.log(accuracySpecified);
      console.log(this.average);
      // return accuracyAreas, accuracyPixel, accuracyLabel;
    }, */
  },
  mounted() {
    // true to get an empty folder for starting labeling process at the backend side
    this.getDataLevel1(true);
