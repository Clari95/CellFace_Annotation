      ...
      <v-col class='mx-auto mt-0 main'>
          <v-row class='canvas-wrapper' v-if="bounce_nextLevel === false">
            ...
            <div class='canvas'>
              ...
              style="font-size:1.2em">
                <b class="teal--text">Level 5: Label 10 images <br/>
                  <p style="font-size:0.8em">Now all funtions are available</p>
                  <p style="font-size:0.5em">Try to label as accurate as possible
                    to get more points</p></b></v-subheader>
                ...
                <v-card dense elevation='10' width="180" class="scores">
                  <v-card-text class='text-center'>
                      ...
                      <v-divider
                        class="mx-4"
                      ></v-divider>
                      <b>Image {{this.imageRef_num - 10}} / 10</b>
                      <!--<br/>
...
export default {
  ...
  methods: {
    ...
    checkLabels(section) {
      ...
      && this.disabled_modi === false && section !== 'last') {
    ...
    // upload labels
    uploadData(section) {
      console.log('uploadData Level 5 start');
      ...
      HTTP.post(
        ...
        `label/${this.labelAreas}/${this.labelAreasRegion}/${this.imageRef_num}/${this.allProperties.UserID}/${this.allProperties.dataset_typ}/${this.allProperties.currentLevel}/${this.allProperties.labelingApproach}`,
      ).then(async (response) => {
        ...
        // to show user how good is at the moment
    ...
    uploadNewData() {
      ...
      HTTP.post(
        // 'http://localhost:3000/labelmask/'
        `newAreas/${this.newAreas}/${this.imageRef_num}/${this.allProperties.UserID}/${this.allProperties.dataset_typ}/5`,
      ).then(async (response) => {
        // this.newImage = response.data.image;
        ...
        if (response.data.wrong_area === false) {
          const points = this.numberCells * 10 + this.numberRegions * 30
            + this.addedAreas * 50 - this.minusPoints;
          this.$emit('updatePoints', points);
        } else {
    ...
    nextImage() {
      ...
      if (((this.imageRef_num - 1) % 10 === 0)) {
        ...
        this.updateScores(this.allProperties.labelingApproach, '');
        // this.updateComponent('next');
        // this.checkLabels('last');
    ...
    getData(mounted) {
      ...
      this.newImage = null;
      this.newImageMask = null;
      const imagenumber = this.imageRef_num;
      HTTP.post(
        `getData/${imagenumber}/${mounted}/${this.allProperties.UserID}/${this.image_section}/${this.allProperties.dataset_typ}/${this.allProperties.labelingApproach}/${this.allProperties.currentLevel}`,
      ).then(async (response) => {
    ...
    updateScores(tooltyp, review) {
      console.log('UPDATE SCORES');
      const wrongAreas = this.minusPoints / 5;
      const AccuracyAll = [0, 0, 0, 0];
      const number = this.imageRef_num - 10;
      const Scores = [number, this.numberCells, this.numberRegions, wrongAreas,
      ...
      HTTP.post(
        ...
        this.nextLevelprops.accuracyForScoring_correct = response.data.accuracy5[2];
        if (review === 'review') {
        //  this.updateBreak();
